...
static NSString* labels_file_name = @"dog_breed_labels";
static NSString* labels_file_type = @"txt";
// These dimensions need to match those the model was trained with.
const int wanted_input_width = 299; // Changed from 224
const int wanted_input_height = 299; // Changed from 224
const int wanted_input_channels = 3;
const float input_mean = 117.0f;
const float input_std = 1.0f;
const std::string input_layer_name = "input_3"; // Changed
const std::string output_layer_name = "softmax0"; // Changed
...
- (void)runCNNOnFrame:(CVPixelBufferRef)pixelBuffer {
  ...
  if (tf_session.get()) {
    ...
    if (!run_status.ok()) {
      ...
      NSLog(@"predition size: %ld labels", predictions.size());
      for (int index = 0; index < predictions.size(); index += 1) {
        const float predictionValue = predictions(index);
        if (predictionValue > 0.05f) {
          std::string label = labels[index % predictions.size()];
          NSLog(@"Predicted Index and Label: %d %s", index, label.c_str());
